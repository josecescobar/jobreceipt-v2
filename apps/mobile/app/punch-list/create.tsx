...
import { useCreatePunchListItem } from '../../src/hooks/usePunchLists';
...
export default function CreatePunchListItemScreen() {
  ...
  const canSubmit = !!jobId && title.trim().length > 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || 'Failed to create punch list item',
      );
    }
  };
  ...
  return (
    <Screen padded={false}>
      <Header title="Add Punch List Item" showBack />
      <KeyboardAvoidingView
